...
export default function TasksPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    if (!session) return
    const [tasksRes, teamRes, userRes, prodsRes] = await Promise.all([
      supabase
        .from('tasks')
  ...
  const formatEventDate = (d: string | null): string => {
    if (!d) return ''
    ...
    <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'flex', gap: '20px', alignItems: 'flex-start' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ background: cardBg, border: `0.5px solid ${border}`, borderRadius: '12px', padding: '16px', marginBottom: '16px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '8px', marginBottom: '8px' }}>
              ...
              <select value={newTask.priority} onChange={e => setNewTask(p => ({ ...p, priority: e.target.value }))} style={inputStyle}>
                {PRIORITIES.map(p => <option key={p} value={p}>{p === 'day of' ? 'Day of event' : p.charAt(0).toUpperCase() + p.slice(1) + ' priority'}</option>)}
              </select>
              <input type="date" value={newTask.due_date} onChange={e => setNewTask(p => ({ ...p, due_date: e.target.value }))} style={inputStyle} />
            </div>
            <div style={{ marginBottom: '12px' }}>
              <select value={newTask.production_id} onChange={e => setNewTask(p => ({ ...p, production_id: e.target.value }))} style={inputStyle}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
              <input
                type="checkbox"
                id="needs_equipment"
                checked={newTask.needs_equipment}
        ...
        <div style={{ display: 'flex', gap: '6px', marginBottom: '8px', flexWrap: 'wrap' }}>
          ...
          <button style={filterBtn(groupBy === 'priority')} onClick={() => setGroupBy('priority')}>By priority</button>
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        padding: '10px 14px',
                        borderBottom: i < groupTasks.length - 1 ? `0.5px solid ${border}` : 'none',
                        background: isSelected
                          ? (dark ? 'rgba(30,108,181,0.12)' : 'rgba(30,108,181,0.06)')
                          : isCompleting
                            ? 'rgba(34,197,94,0.06)'
                            : 'transparent',
                        ...
                        borderLeft: isSelected ? '3px solid #1e6cb5' : '3px solid transparent',
                      }}
                    ...
                    >
                      ...
                      <button
                        ...
                        style={{
                          ...
                          border: `1.5px solid ${isCompleting || task.status === 'complete' ? '#22c55e' : task.status === 'in progress' ? '#f59e0b' : border}`,
                          background: isCompleting || task.status === 'complete' ? '#22c55e' : task.status === 'in progress' ? 'rgba(245,158,11,0.15)' : 'transparent',
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0 }}>
                        {isCompleting ? (
                          ...
                          <>
                            {task.priority !== 'normal' && (
                              <span style={{ fontSize: '10px', padding: '2px 7px', borderRadius: '6px', background: priorityStyle.bg, color: priorityStyle.color, whiteSpace: 'nowrap' as const }}>
                        ...
                        {assignee ? (
                          ...
                          <div style={{ width: '22px', height: '22px', borderRadius: '50%', border: `1.5px dashed ${border}`, flexShrink: 0 }} />
                        )}
                      </div>
                    </div>
                  )
      ...
      {selectedTask && (
        <div style={{
          width: '340px', flexShrink: 0, position: 'sticky', top: '80px',
          background: panelBg, border: `0.5px solid ${border}`, borderRadius: '14px',
          ...
          <div style={{ padding: '16px' }}>
            {/* Title */}
            <p style={{ fontSize: '16px', fontWeight: 600, color: text, margin: '0 0 16px', lineHeight: 1.3 }}>{selectedTask.title}</p>
            ...
            {/* Equipment flag */}
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px', padding: '10px 12px', background: selectedTask.needs_equipment ? 'rgba(249,115,22,0.1)' : inputBg, borderRadius: '8px', border: `0.5px solid ${selectedTask.needs_equipment ? 'rgba(249,115,22,0.3)' : border}`, cursor: 'pointer' }}
              onClick={() => updateTask(selectedTask.id, { needs_equipment: !selectedTask.needs_equipment })}
            >
              <input
                type="checkbox"
                checked={selectedTask.needs_equipment}
                onChange={() => {}}
