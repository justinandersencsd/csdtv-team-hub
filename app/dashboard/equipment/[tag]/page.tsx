...
export default function EquipmentDetailPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    if (eqData) {
      ...
      setEditModel(eqData.model || '')
      setEditSerial(eqData.serial_number || '')
      setEditSite(eqData.site || 'District Office')
      setEditCondition(eqData.condition || 'good')
      setEditNotes(eqData.notes || '')
      setEditStatus(eqData.status)

      // Loans for this item
      const { data: loanData } = await supabase
        .from('equipment_loans')
        .select('*, checked_out_by_user:team!equipment_loans_checked_out_by_fkey(name), checked_in_by_user:team!equipment_loans_checked_in_by_fkey(name)')
      ...
      const { data: actData } = await supabase
        ...
        .select('*, user:team!equipment_activity_user_id_fkey(name)')
        .eq('equipment_id', eqData.id)
        .order('created_at', { ascending: false })
      if (actData) setActivity(actData as any)

      // Kits this item belongs to
      const { data: kitItemData } = await supabase
        .from('equipment_kit_items')
        .select('kit_id, kit:equipment_kits(id, name)')
        .eq('equipment_id', eqData.id)
      if (kitItemData) setKits((kitItemData as any).map((ki: any) => ki.kit).filter(Boolean))

      // All kits for the add-to-kit dropdown
      const { data: allKitData } = await supabase.from('equipment_kits').select('id, name').order('name')
      if (allKitData) setAllKits(allKitData)
    }
  ...
  const conditionColor = (c: string) => {
    switch (c) {
      ...
      case 'needs_repair': return '#f97316'
  ...
  const handleCheckout = useCallback(async () => {
    ...
    await supabase.from('equipment_loans').insert({
      equipment_id: item.id, borrower_name: borrowerName.trim(),
    ...
    await supabase.from('equipment_activity').insert({
      equipment_id: item.id, action: 'checked_out',
      detail: `Checked out to ${borrowerName.trim()}${dueDate ? ` — due ${dueDate}` : ''}`, user_id: user.id,
    })
  ...
  const handleCheckin = useCallback(async () => {
    ...
    await supabase.from('equipment_activity').insert({
      equipment_id: item.id, action: 'checked_in',
      detail: `Returned by ${activeLoan.borrower_name}`, user_id: user.id,
    })
    loadData()
  ...
  const handleSaveEdit = useCallback(async () => {
    ...
    const updates: any = {
      ...
      serial_number: editSerial || null, site: editSite, condition: editCondition,
      notes: editNotes || null, status: editStatus, updated_at: new Date().toISOString(),
    }
    await supabase.from('equipment').update(updates).eq('id', item.id)
    if (editStatus !== item.status) {
      await supabase.from('equipment_activity').insert({
        equipment_id: item.id, action: 'status_changed',
        detail: `Status changed from ${item.status} to ${editStatus}`, user_id: user.id,
      })
    }
    await supabase.from('equipment_activity').insert({
      equipment_id: item.id, action: 'edited', detail: 'Item details updated', user_id: user.id,
    })
    setEditing(false)
    loadData()
  ...
  const handleAddToKit = useCallback(async () => {
    ...
    await supabase.from('equipment_activity').insert({
      equipment_id: item.id, action: 'added_to_kit',
      detail: `Added to kit: ${allKits.find(k => k.id === selectedKit)?.name}`, user_id: user.id,
    })
  ...
  const handleRemoveFromKit = useCallback(async (kitId: string, kitName: string) => {
    ...
    await supabase.from('equipment_activity').insert({
      equipment_id: item.id, action: 'removed_from_kit', detail: `Removed from kit: ${kitName}`, user_id: user.id,
    })
    loadData()
  }, [item, user, supabase, loadData])
  ...
  return (
    <div style={{ maxWidth: '1600px', margin: '0 auto', padding: '24px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          ...
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
            <span style={{ display: 'inline-block', padding: '4px 12px', borderRadius: '20px', fontSize: '12px', fontWeight: 600, background: sc.bg, color: sc.text }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          ...
          {item.status === 'checked_out' && activeLoan && (
            <button onClick={handleCheckin} style={{
              background: '#22c55e', border: 'none', borderRadius: '10px', color: '#fff',
              padding: '10px 20px', fontSize: '14px', cursor: 'pointer', fontWeight: 600, fontFamily: 'inherit', minHeight: '44px',
            }}>Check In</button>
          )}
          {isManager && !editing && (
      ...
      {activeLoan && (
        <div style={{
          background: dark ? '#3b2a15' : '#fef3c7', borderRadius: '12px', padding: '14px 20px',
          marginBottom: '16px', border: `1px solid ${dark ? '#5a3a10' : '#fcd34d'}`,
          ...
          <div style={{ fontSize: '14px', color: dark ? '#fbbf24' : '#92400e' }}>
            ...
            {activeLoan.due_date && (
              <span style={{ color: new Date(activeLoan.due_date) < new Date() ? '#ef4444' : 'inherit' }}>
                {' · '}Due {new Date(activeLoan.due_date).toLocaleDateString()}
                {new Date(activeLoan.due_date) < new Date() && ' (OVERDUE)'}
              </span>
            )}
          </div>
        </div>
      ...
      <div style={{ display: 'flex', gap: '4px', marginBottom: '16px', borderBottom: `1px solid ${border}` }}>
        {(['info', 'history', 'activity'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)} style={{
            ...
            color: tab === t ? '#5ba3e0' : muted, cursor: 'pointer', fontFamily: 'inherit', textTransform: 'capitalize',
          }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '16px' }}>
          <div style={{ background: cardBg, borderRadius: '14px', padding: '20px', border: `1px solid ${border}` }}>
            ...
            {[
              ...
              { label: 'Subcategory', value: getCategoryName(item.subcategory_id) },
              { label: 'Status', value: item.status.replace('_', ' ') },
              { label: 'Condition', value: item.condition.replace('_', ' ') },
            ...
            {item.notes && (
              <div style={{ marginTop: '12px' }}>
                <div style={{ fontSize: '13px', color: muted, marginBottom: '4px' }}>Notes</div>
                <div style={{ fontSize: '14px', color: text }}>{item.notes}</div>
              </div>
            )}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div style={{ background: cardBg, borderRadius: '14px', padding: '20px', border: `1px solid ${border}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                ...
                {isManager && (
                  <button onClick={() => setShowKitAdd(!showKitAdd)} style={{
                    background: 'none', border: 'none', color: '#5ba3e0', cursor: 'pointer', fontSize: '13px', fontFamily: 'inherit',
                  }}>{showKitAdd ? 'Cancel' : '+ Add to Kit'}</button>
                )}
              </div>
              {showKitAdd && (
                <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
                  <select value={selectedKit} onChange={e => setSelectedKit(e.target.value)} style={{ ...inputStyle, flex: 1 }}>
                    ...
                    {allKits.filter(k => !kits.find(mk => mk.id === k.id)).map(k => (
                      <option key={k.id} value={k.id}>{k.name}</option>
                    ))}
                  </select>
                  <button onClick={handleAddToKit} disabled={!selectedKit} style={{
                    background: selectedKit ? '#1e6cb5' : '#333', border: 'none', borderRadius: '10px',
                    color: '#fff', padding: '10px 16px', fontSize: '13px', cursor: selectedKit ? 'pointer' : 'default',
                    fontWeight: 600, fontFamily: 'inherit', minHeight: '44px', whiteSpace: 'nowrap',
                  }}>Add</button>
                </div>
              )}
              {kits.length === 0 && !showKitAdd && (
                <div style={{ fontSize: '13px', color: muted }}>Not in any kit</div>
              )}
              {kits.map(kit => (
                <div key={kit.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: `1px solid ${border}` }}>
                  ...
                  {isManager && (
                    <button onClick={() => handleRemoveFromKit(kit.id, kit.name)} style={{
            ...
            <div style={{ background: cardBg, borderRadius: '14px', padding: '20px', border: `1px solid ${border}` }}>
              ...
              <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
                <div>
                  <div style={{ fontSize: '22px', fontWeight: 700, color: '#5ba3e0' }}>{loans.length}</div>
                  <div style={{ fontSize: '12px', color: muted }}>Total Loans</div>
                </div>
                <div>
                  <div style={{ fontSize: '22px', fontWeight: 700, color: '#f59e0b' }}>{loans.filter(l => !l.checked_in_at).length}</div>
                  <div style={{ fontSize: '12px', color: muted }}>Active</div>
                </div>
                <div>
                  <div style={{ fontSize: '22px', fontWeight: 700, color: text }}>{activity.length}</div>
                  <div style={{ fontSize: '12px', color: muted }}>Activities</div>
                </div>
              </div>
            </div>
          </div>
      ...
      {tab === 'info' && editing && (
        <div style={{ background: cardBg, borderRadius: '14px', padding: '24px', border: `1px solid ${border}`, maxWidth: '600px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <div>
              <label style={{ fontSize: '13px', color: muted, display: 'block', marginBottom: '4px' }}>Name</label>
              <input value={editName} onChange={e => setEditName(e.target.value)} style={inputStyle} />
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              <div>
                <label style={{ fontSize: '13px', color: muted, display: 'block', marginBottom: '4px' }}>Brand</label>
                <input value={editBrand} onChange={e => setEditBrand(e.target.value)} style={inputStyle} />
              </div>
              <div>
                <label style={{ fontSize: '13px', color: muted, display: 'block', marginBottom: '4px' }}>Model</label>
                <input value={editModel} onChange={e => setEditModel(e.target.value)} style={inputStyle} />
              </div>
            </div>
            <div>
              <label style={{ fontSize: '13px', color: muted, display: 'block', marginBottom: '4px' }}>Serial Number</label>
              <input value={editSerial} onChange={e => setEditSerial(e.target.value)} style={inputStyle} />
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
              <div>
                <label style={{ fontSize: '13px', color: muted, display: 'block', marginBottom: '4px' }}>Status</label>
              ...
              <div>
                <label style={{ fontSize: '13px', color: muted, display: 'block', marginBottom: '4px' }}>Site</label>
                <select value={editSite} onChange={e => setEditSite(e.target.value)} style={inputStyle}>
              ...
              <div>
                <label style={{ fontSize: '13px', color: muted, display: 'block', marginBottom: '4px' }}>Condition</label>
            ...
            <div>
              <label style={{ fontSize: '13px', color: muted, display: 'block', marginBottom: '4px' }}>Notes</label>
              <textarea value={editNotes} onChange={e => setEditNotes(e.target.value)} rows={3} style={{ ...inputStyle, minHeight: '80px', resize: 'vertical' as const }} />
            </div>
          </div>
          <div style={{ display: 'flex', gap: '8px', marginTop: '16px' }}>
            <button onClick={handleSaveEdit} style={{
              background: '#1e6cb5', border: 'none', borderRadius: '10px', color: '#fff',
              padding: '10px 24px', fontSize: '14px', cursor: 'pointer', fontWeight: 600, fontFamily: 'inherit', minHeight: '44px',
            }}>Save</button>
            <button onClick={() => setEditing(false)} style={{
              background: 'none', border: `1px solid ${border}`, borderRadius: '10px', color: muted,
              padding: '10px 24px', fontSize: '14px', cursor: 'pointer', fontFamily: 'inherit', minHeight: '44px',
            }}>Cancel</button>
          </div>
        </div>
      )}
      ...
      {tab === 'history' && (
        <div style={{ background: cardBg, borderRadius: '14px', border: `1px solid ${border}`, overflow: 'hidden' }}>
          {loans.length === 0 && (
            <div style={{ padding: '40px', textAlign: 'center', color: muted, fontSize: '14px' }}>No loan history</div>
          )}
          {loans.map((loan, i) => {
              ...
              <div key={loan.id} style={{ padding: '14px 20px', borderBottom: i < loans.length - 1 ? `1px solid ${border}` : 'none', borderLeft: isActive ? '3px solid #f59e0b' : '3px solid transparent' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                  <div style={{ fontSize: '14px', fontWeight: 500, color: text }}>
                    {loan.borrower_name}
                    {loan.borrower_info && <span style={{ color: muted, fontWeight: 400 }}> ({loan.borrower_info})</span>}
                  </div>
                  <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                    {isActive && (
                      <span style={{ padding: '2px 8px', borderRadius: '20px', fontSize: '10px', fontWeight: 700, background: isOverdue ? (dark ? '#3b1515' : '#fee2e2') : (dark ? '#3b2a15' : '#fef3c7'), color: isOverdue ? '#ef4444' : '#f59e0b', textTransform: 'uppercase' }}>
                        {isOverdue ? 'Overdue' : 'Active'}
                      </span>
                    )}
                    {!isActive && (
                      <span style={{ padding: '2px 8px', borderRadius: '20px', fontSize: '10px', fontWeight: 600, background: dark ? '#16382a' : '#dcfce7', color: '#22c55e' }}>Returned</span>
                    )}
                  </div>
                </div>
                <div style={{ fontSize: '12px', color: muted }}>
                  ...
                  {loan.condition_on_return && ` · Condition: ${loan.condition_on_return}`}
                </div>
                {loan.notes && (
                  <div style={{ fontSize: '12px', color: muted, marginTop: '4px', fontStyle: 'italic' }}>{loan.notes}</div>
                )}
              </div>
            )
          })}
      ...
      {tab === 'activity' && (
        <div style={{ background: cardBg, borderRadius: '14px', border: `1px solid ${border}`, overflow: 'hidden' }}>
          {activity.length === 0 && (
            <div style={{ padding: '40px', textAlign: 'center', color: muted, fontSize: '14px' }}>No activity yet</div>
          )}
          {activity.map((a, i) => (
            <div key={a.id} style={{ padding: '12px 20px', borderBottom: i < activity.length - 1 ? `1px solid ${border}` : 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ fontSize: '11px', color: muted, whiteSpace: 'nowrap', marginLeft: '12px' }}>
                {(a.user as any)?.name || ''} · {new Date(a.created_at).toLocaleDateString()}
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000, padding: '20px' }}>
          <div style={{ background: dark ? '#111827' : '#fff', borderRadius: '16px', padding: '28px', maxWidth: '440px', width: '100%', border: `1px solid ${border}` }}>
            ...
            <div style={{ display: 'flex', gap: '8px', marginTop: '20px' }}>
              <button onClick={handleCheckout} disabled={!borrowerName.trim()} style={{
